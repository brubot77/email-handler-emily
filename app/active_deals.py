...
def _link_property_tab_to_active_deals_row(
    ...
    - Set O16 Price equal to Active Deals Latest Offer
    - Set B5 Down Payment % based on 80% of Appraisal Est. / Latest Offer
    - Set Active Deals ConC equal to property tab P16 Cash on Cash %
    """
    ...
    # Shannon Price column is O16.
    if latest_offer_col:
        property_ws["O16"] = f"={latest_offer_ref}"
        property_ws["O16"].number_format = '$#,##0.00;[Red]($#,##0.00)'

    # Shannon Down Payment % input is B5.
    # Formula requested:
    # 1 - (0.8 * Active Deals Appraisal Est. / Active Deals Latest Offer)
    if latest_offer_col and appraisal_est_col:
        property_ws["B5"] = (
            f'=IFERROR(1-(0.8*{appraisal_est_ref}/{latest_offer_ref}),0.2)'
        )
        property_ws["B5"].number_format = '0.00%'

    # Active Deals ConC should equal property tab Cash on Cash % at P16.
    if conc_col:
        active_ws.cell(
            row=row_idx,
            column=conc_col,
            value=f"={_get_cell_ref(property_sheet_name, 16, 16)}",
        )
        active_ws.cell(row=row_idx, column=conc_col).number_format = '0.00%'
